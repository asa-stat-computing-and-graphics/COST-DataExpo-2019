...
df = pd.read_csv('data/dataset_raw.csv', index_col=0)
...
walls = walls.drop(walls[walls["_d3"] == 8].index)
# "8" represents the absence of response
...
windows = windows.drop(windows[windows["_e1"] == 8].index)
# "8" represents the absence of response
...
stairways = stairways.drop(stairways[stairways["_f1"] == 8].index)
# "8" represents the absence of response
...
floors = floors.drop(floors[floors["_g3"] == 8].index)
# "8" represents the absence of response
...
severity = pd.concat([walls["severity_walls"], windows["severity_windows"],
                      stairways['severity_stairways'],
                      floors['severity_floors']], axis=1)
...
df_cond["pca_scaled"] = np.interp(df_cond["pca"],
                                  (df_cond["pca"].min(),
                                   df_cond["pca"].max()), (0, 10))


# we create a Pandas Dataframe that contains the predictors
...
# we create a Pandas Dataframe that contains the predictors
# used in the analysis, the housing conditions and the PCA scores
final_df = pd.DataFrame()
final_df = df_pred.join(df_cond)

# exporting of the dataset
final_df.to_csv('data/dataset_processed.csv', index=False)
